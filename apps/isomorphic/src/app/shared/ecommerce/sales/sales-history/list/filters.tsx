...
interface FilterProps {
  ...
  handleSalesManFilter: (value?: number) => void;
}
...
}: TableToolbarProps<TData> & FilterProps) {
  ...
  return (
    <Flex className="flex-col sm:flex-row">
      <SalesmanSelectionField
        ...
        onClear={() => handleSalesManFilter(undefined)}
      />
...
function FilterElements<T extends Record<string, any>>({
  ...
  const priceFieldValue = (table.getColumn('price')?.getFilterValue() ?? [
    '',
    '',
  ]) as string[];
  const createdDate =
  ...
  return (
    <>
      <PriceField
        ...
        onChange={(v) => table.getColumn('price')?.setFilterValue(v)}
        label="price"
      />
      <DateFiled
        selectsRange
